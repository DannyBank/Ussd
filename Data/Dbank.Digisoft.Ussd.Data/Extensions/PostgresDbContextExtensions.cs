...
namespace Dbank.Digisoft.Ussd.Data.Extensions
{
    public static class PostgresDbContextExtensions
    {
        ...
        {
            var dr = dataReader as NpgsqlDataReader;
        ...
        public static async Task<string> ExecuteReturningString(this NpgsqlCommand command) 
        {
            using (command)
            {
                if (command.Connection != null && command.Connection.State == ConnectionState.Closed)
                ...
                try
                {
                    var data = (await command.ExecuteScalarAsync()).ToString();
                    return data;
                }
                finally
                {
        ...
        public static async Task ExecuteNonReturningAsync(this NpgsqlCommand command)
        {
            using (command)
            {
                if (command.Connection != null && command.Connection.State == ConnectionState.Closed)
